...
static void verif_option(int argc, char **argv)
{
   ...
   if (access(all_option.filepath, R_OK))
      erreur("Pas de fichier '%s'", all_option.filepath);
...
void decode_baseline_image(FILE *infile, img_t *img)
{
   ...
   if (nbcomp == 3)
   {
      ...
      {
         if (img->comps->comps[0]->idc == img->comps->ordre[i])
            y_id = i;
         if (img->comps->comps[1]->idc == img->comps->ordre[i])
            cb_id = i;
...
int main(int argc, char *argv[])
{
   ...
   if (img->section->num_sof == 0)
      decode_baseline_image(fichier, img);
   else if (img->section->num_sof == 2)
      decode_progressive_image(fichier, img);
   else
      erreur("sof%d non supporté", img->section->num_sof);
