...
static char my_getc(FILE *file, char old, bool *stop);

// Retourne la valeur obtenue en lisant <magnitude> bits à l'adresse
// <file>+<off>. <c> est la valeur de l'octet courant.
static int16_t read_val_from_magnitude(FILE* file, uint8_t magnitude, uint8_t *off, char *c, bool *stop);
static uint16_t read_indice(FILE* file, uint8_t nb_bit, uint8_t *off, char *c, bool *stop);
...
static int16_t decode_coef_DC(FILE *file, huffman_tree_t *symb_decode, uint8_t *off, char *c, bool *stop);
...
static uint16_t decode_coef_AC(FILE *file, uint8_t num_sof, huffman_tree_t *symb_decode, blocl16_t *sortie, uint64_t *resi, uint8_t *off, char *c, bool *stop);
...
static uint16_t decode_list_coef(FILE* file, huffman_tree_t* ht, uint8_t num_sof, blocl16_t *sortie, uint8_t *off, enum acdc_e type, uint8_t s_start, uint8_t s_end, bool *stop);
...
static uint16_t read_indice(FILE* file, uint8_t nb_bit, uint8_t *off, char *c, bool *stop) {
   if (nb_bit + *off < 8) {
      ...
      *off += nb_bit;
      ...
      if (nb_bit + *off >= 16) printf("%x ", *c);
      //c = fgetc(file);
      *c = my_getc(file, *c, stop);
      if (*stop) return 0;
      if (nb_bit + *off < 16) {
...
static int16_t read_val_from_magnitude(FILE* file, uint8_t magnitude, uint8_t *off, char *c, bool *stop) {
   uint16_t indice = read_indice(file, magnitude, off, c, stop);
   if (*stop) return 0;
   return get_val_from_magnitude(magnitude, indice);
}

static int16_t decode_coef_DC(FILE *file, huffman_tree_t *symb_decode, uint8_t *off, char *c, bool *stop) {
   return read_val_from_magnitude(file, symb_decode->symb, off, c, stop);
}
...
static uint16_t decode_coef_AC(FILE *file, uint8_t num_sof, huffman_tree_t *symb_decode, blocl16_t *sortie, uint64_t *resi, uint8_t *off, char *c, bool *stop) {
   if (symb_decode->symb == 0xf0) {  // RLE
	 ...
	 *resi += alpha;
	 sortie->data[(*resi)] = read_val_from_magnitude(file, gamma, off, c, stop);
	 if (*stop) return 0;
	 (*resi)++;
      }
   }
	 ...
	 if (symb_decode->fils[1] == NULL && symb_decode->fils[0] == NULL) {
	    if (type == DC) {
		  ...
		  erreur("Le code de huffman avec que des 1 est utilisé\n");
	       ...
	       sortie->data[resi] = decode_coef_DC(file, symb_decode, &i, &c, stop);
	       if (*stop) return 0;
	       code_que_un = true;
	       resi++;
	    } else {
